...
def compute_topic_modelling(corpus, dictionary, num_topics, workers, passes):
    return LdaMulticore(corpus=corpus, id2word=dictionary, iterations=10, num_topics=num_topics, workers=workers, passes=passes)

def get_model_lda_and_k_optimal(spacy_nlp, df, no_below, no_above, keep_n, workers, passes, max_topics, type_coherence):
    ...
    for i in range(1,max_topics):
        lda_model = LdaMulticore(corpus=corpus, id2word=dictionary, iterations=10, num_topics=i, workers=workers, passes=passes, random_state=100)
...
def get_coherence(lda_model, corpus, dictionary, type_coherence, tokens):
    ...
    if type_coherence == 'u_mass':
        cm = CoherenceModel(model=lda_model, corpus=corpus, dictionary=dictionary, coherence=type_coherence)
    else:
        cm = CoherenceModel(model=lda_model, texts=tokens, corpus=corpus, dictionary=dictionary, coherence='c_v')
    return cm.get_coherence()

def topic_modelling_pipeline(spacy_nlp, df, no_below, no_above, keep_n, workers, passes, num_topics):
    tokenized_df = tokenize_dataset(df, spacy_nlp)
    tokens = tolist(tokenized_df)
    dictionary = get_dictionary_from_tokens(tokens, no_below, no_above, keep_n)
    corpus = [dictionary.doc2bow(doc) for doc in tokens]
